...
public class CoTermNSStripeCombiner {
...
//						stripe.put(new Text(line.get(j).toString()), new IntWritable(count));
	...
	public static class NSStripeCombiner extends Reducer<Text, MapWritable, Text, MapWritable>{
		private MapWritable interStripe = new MapWritable();
		...
		public void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException{
			...
			context.write(key, interStripe);
			
			
//			for(Entry<Writable, Writable> entry: interStripe.entrySet()){
...
//			}
		...
		private void addAll(MapWritable val){
			Set<Entry<Writable, Writable>> sets = val.entrySet();
			
			int sum = 0;
			for(Entry<Writable, Writable> entry: sets){
				Writable k = entry.getKey();
				if(interStripe.containsKey(new Text(k.toString()))){
					sum = ((IntWritable)interStripe.get(new Text(k.toString()))).get() + ((IntWritable)entry.getValue()).get();
					interStripe.put(new Text(k.toString()), new IntWritable(sum));
//					System.out.println(sum);
				}else{
//					System.out.println(entry.getValue());
					interStripe.put(new Text(k.toString()), (IntWritable)entry.getValue());
				}
			}
		}
	}
